...
car = pygame.image.load('car.png') #load sprite
...
while 1:
	...
	for event in pygame.event.get():
		...
		elif event.key == K_UP: k_up * 2
		elif event.key == K_DOWN: k_down * -2
	...
	# speed += (k_up + k_down)
	# if speed > MAX_FOWARD_SPEED : speed = MAX_FOWARD_SPEED
	# if speed < MAX_REVERSE_SPEED : speed = MAX_REVERSE_SPEED
	direction += (k_right + k_left)
	#.. new position based on curent position,speed and direction
	x , y = position
